...
func (s *Service) Create(manualActivity ManualActivity) (ManualActivity, error) {
	...
	err := s.db.QueryRow(
		...
		(user_id, activity_id, name, description, estimated_time, location_id, scheduled_at, is_active) 
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8) 
		RETURNING id`,
		manualActivity.UserID, manualActivity.ActivityID, manualActivity.Name,
		manualActivity.Description, manualActivity.EstimatedTime, manualActivity.LocationID, manualActivity.ScheduledAt, manualActivity.IsActive,
	).Scan(&manualActivity.ID)
...
func (s *Service) ReadAll() ([]ManualActivity, error) {
	...
	rows, err := s.db.Query(context.Background(),
		"SELECT id, user_id, activity_id, name, description, estimated_time::text, location_id, scheduled_at, is_active FROM manual_activities")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(
			...
			&manualActivity.ScheduledAt, &manualActivity.IsActive)
		if err != nil {
			return nil, err
		}
...
func (s *Service) Read(id string) (ManualActivity, bool, error) {
	...
	err := s.db.QueryRow(
		context.Background(),
		`SELECT id, user_id, activity_id, name, description, estimated_time::text, location_id, scheduled_at, is_active 
...
func (s *Service) Update(id string, manualActivity ManualActivity) (ManualActivity, bool, error) {
	...
	cmdTag, err := s.db.Exec(
		...
		estimated_time = $5, location_id = $6, scheduled_at = $7, is_active = $8 
		WHERE id = $9`,
		manualActivity.UserID, manualActivity.ActivityID, manualActivity.Name,
		manualActivity.Description, manualActivity.EstimatedTime, manualActivity.LocationID,
		manualActivity.ScheduledAt, manualActivity.IsActive, id)
