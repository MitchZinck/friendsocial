...
func (s *Service) ReadAll() ([]ManualActivity, error) {
	...
	rows, err := s.db.Query(context.Background(),
		"SELECT id, user_id, activity_id, name, description, estimated_time::text, location_id, created_at::text, is_active FROM manual_activities")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *Service) Read(id string) (ManualActivity, bool, error) {
	...
	err := s.db.QueryRow(
		context.Background(),
		`SELECT id, user_id, activity_id, name, description, estimated_time::text, location_id, created_at::text, is_active 
